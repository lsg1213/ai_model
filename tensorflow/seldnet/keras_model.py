...
import efficientnet.model as efficientnet
...
def da_get_model(data_in, data_out, dropout_rate, nb_cnn2d_filt, pool_size,
    ...
    for nb_fnn_filt in fnn_size:
        sed = TimeDistributed(Dense(nb_fnn_filt))(sed)
        sed = Dropout(dropout_rate)(sed)
    sed = TimeDistributed(Dense(2))(sed)
    sed = Activation('sigmoid', name='sed_out')(sed)
    ...
    # doa = sed[:,:,-1][...,tf.newaxis] * doa # v2

    # doa = Dense(11)(doa)
    doa = TimeDistributed(Dense(classes))(doa)
    doa_s = Bidirectional(GRU(32))(doa)
    doa_s = Dense(classes)(doa_s)
    doa_s = Activation('softmax', name='doa_s_out')(doa_s)
    doa = Activation('softmax', name='doa_out')(doa)
    ...
    model = Model(inputs=spec_start, outputs=[sed, doa, doa_s])
    model.compile(optimizer=Adam(learning_rate=0.001), loss=['categorical_crossentropy', 'categorical_crossentropy', 'categorical_crossentropy'], loss_weights=weights,
    metrics={'sed_out':'AUC', 'doa_out': tf.keras.metrics.MeanAbsoluteError(), 'doa_s_out': tf.keras.metrics.MeanAbsoluteError()})

    model.summary()
    return model

def reg_model(data_in, data_out, dropout_rate, nb_cnn2d_filt, pool_size,
                                rnn_size, fnn_size, classification_mode, weights):
    # model definition
    spec_start = Input(shape=(data_in[-3], data_in[-2], data_in[-1]))
    spec_cnn = spec_start
    ...
    model = Model(inputs=spec_start, outputs=[sed, doa, doa_s])
    model.compile(optimizer=Adam(learning_rate=0.001), loss=['binary_crossentropy', 'sparse_categorical_crossentropy', 'sparse_categorical_crossentropy'], loss_weights=weights,
    metrics={'sed_out':'AUC', 'doa_out': tf.keras.metrics.MeanAbsoluteError(), 'doa_s_out': tf.keras.metrics.MeanAbsoluteError()})
...
def reg_model(data_in, data_out, dropout_rate, nb_cnn2d_filt, pool_size,
                                rnn_size, fnn_size, classification_mode, weights):
    inputs = Input(shape=(data_in[-3], data_in[-2], data_in[-1]))
    model = getattr(efficientnet, 'EfficientNetB0')(include_top=False,weights=None,input_shape=tf.TensorShape((data_in[-3], data_in[-2], data_in[-1])),classes=11, backend=tf.keras.backend,layers=tf.keras.layers,models=tf.keras.models,utils=tf.keras.utils)(inputs)
    tf.keras.Model(i)
    pdb.set_trace()
    return model