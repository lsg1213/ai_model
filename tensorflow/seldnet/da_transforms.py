...
def merge_complex_specs(background, 
    ...
    if noise is not None:
        if tf.random.uniform([]) < noise_prob:
            n_ratio = tf.math.pow(10., -tf.random.uniform([], maxval=2)) # SNR0~-20
            frame = tf.cast(tf.shape(noise)[time_axis], tf.float32)
            if frame < n_frame:
                noise = tf.pad(
                    noise,
                    [[0, 0] 
                     if i != time_axis 
                     else [n_frame - tf.cast(min_noise_ratio*frame, tf.int32)]*2
                     for i in range(n_dims)])
...
def wav_to_complex(wav, n_fft=512, hop=256):
    '''
    INPUT
        wav: [chan, frames]
