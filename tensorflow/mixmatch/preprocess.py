...
def split_dataset(dataset, num_labelled, num_validations, num_confusion, num_classes):
    ...
    counter = [0 for _ in range(num_classes)]
    labelled = labelled.shuffle(buffer_size=10000)
    _labelled = []
    for example in iter(labelled):
        label = int(example['label'])
        counter[label] += 1
        if counter[label] <= (num_confusion / num_classes):
    ...
    return labelled, unlabelled, validation, confusion
...
def fetch_dataset(args, log_dir):
    ...
    # creating datasets
    if any([not os.path.exists(f'{dataset_path}/{split}.tfrecord') for split in ['trainX', 'trainU', 'validation', 'confusion', 'test']]):
        train, test = download_dataset(dataset_name=args.dataset)

        trainX, trainU, validation, confusion = split_dataset(train, args.labelled_examples, args.validation_examples, args.conf_num, num_classes)

        for name, dataset in [('trainX', trainX), ('trainU', trainU), ('validation', validation), ('test', test), ('confusion', confusion)]:
        ...
        export_tfrecord_dataset(f'{dataset_path}/confusion.tfrecord', confusion)
    ...
    parsed_confusion = load_tfrecord_dataset(f'{dataset_path}/confusion.tfrecord')
    ...
    confusion = process_parsed_dataset(parsed_confusion, num_classes)

    return trainX, trainU, validation, test, confusion, num_classes
